...
function PortfolioLayout() {
  ...
  const [lastAdminVisit, setLastAdminVisit] = useState<number | null>(() => {
    const saved = localStorage.getItem("lastAdminVisit");
    return saved ? parseInt(saved, 10) : null;
  });
  const { track } = useAnalytics();
  const { toast } = useToast();
  const [showAdminLink, setShowAdminLink] = useState(localStorage.getItem("admin_access_unlocked") === "true");
  ...
  const handleNavClick = (section: string) => {
    if (activeSection === "Admin" && section !== "Admin") {
      const now = Date.now();
      setLastAdminVisit(now);
      localStorage.setItem("lastAdminVisit", now.toString());
    }
    if (section === "Admin") {
      setLastAdminVisit(null);
      localStorage.removeItem("lastAdminVisit");
    }
    setActiveSection(section);
    setMobileMenuOpen(false);
  };

  // Auto-logout: If away from Admin for > 2 mins, clear session
  useEffect(() => {
    const checkAuth = () => {
      const token = localStorage.getItem("adminToken");
      if (token && activeSection !== "Admin" && lastAdminVisit) {
        const awayTime = Date.now() - lastAdminVisit;
        if (awayTime > 120000) { // 2 minutes
          localStorage.removeItem("adminToken");
          localStorage.removeItem("lastAdminVisit");
          setLastAdminVisit(null);
          console.log("Admin session cleared due to inactivity away from page.");
        }
      }
    };
  ...
  }, [activeSection, lastAdminVisit]);

  // Secret keyboard shortcut (Ctrl+Shift+Alt+A) to toggle admin link
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.ctrlKey && e.shiftKey && e.altKey && (e.key === "a" || e.key === "A")) {
        const newValue = !showAdminLink;
        setShowAdminLink(newValue);
        localStorage.setItem("admin_access_unlocked", newValue.toString());
        toast({
          title: newValue ? "Admin Access Revealed" : "Admin Access Hidden",
          description: newValue 
            ? "The hidden admin button is now available in the footer." 
            : "The admin button has been removed from the footer."
        });
      }
    };
    ...
    switch (activeSection) {
      case "Home":    return (
        <div className="space-y-24">
          <HomeSection navigateTo={handleNavClick} />
          <AutomationSection navigateTo={handleNavClick} />
        </div>
      );
